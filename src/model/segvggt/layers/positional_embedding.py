...
class PositionEmbeddingCoordsSine(nn.Module):
    ...
    def get_sine_embeddings(self, xyz, num_channels, input_range=None, modulated=None):
        ...
        if modulated is not None:
            assert modulated.shape == xyz.shape
    ...
    def get_fourier_embeddings(self, xyz, num_channels=None, input_range=None, modulated=None):
        ...
        if self.normalize:
            xyz = shift_scale_points(xyz, src_range=input_range)
        if modulated is not None:
            assert modulated.shape[0] == bsize and modulated.shape[-1] == 3
            assert isinstance(modulated, torch.Tensor)
            xyz = xyz / modulated
    ...
    def forward(self, xyz, num_channels=None, input_range=None, modulated=None):
        ...
        if self.pos_type == "sine":
            if modulated is not None:
                return self.get_sine_embeddings(xyz, num_channels, input_range, modulated)
            else:
                with torch.no_grad():
                    return self.get_sine_embeddings(xyz, num_channels, input_range, modulated)
        elif self.pos_type == "fourier":
            with torch.no_grad():
                return self.get_fourier_embeddings(xyz, num_channels, input_range, modulated)
        else:
...
def shift_scale_points(pred_xyz, src_range, dst_range=None):
    ...
    if dst_range is None:
        dst_range = [
            torch.zeros((src_range[0].shape[0], pred_xyz.shape[-1]), device=src_range[0].device),
            torch.ones((src_range[0].shape[0], pred_xyz.shape[-1]), device=src_range[0].device),
        ]
