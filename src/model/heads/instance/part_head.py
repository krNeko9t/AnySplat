"""PartHead – IGGT-style instance embedding head.

Adapted from IGGT/iggt/heads/part_head.py.  Key design:
  * DPT-style multi-scale fusion (scratch / refinenet)
  * Cross-attention with point features at 2 scales
  * Window self-attention (SwinSA / HAB) for spatial context
  * Window cross-attention (SwinCA / OCAB) at one scale

The head outputs dense per-pixel instance embeddings of shape
[B, S, output_dim, H, W].
"""
...
# ---------------------------------------------------------------------------
# DPT fusion helpers (self-contained, no base-class dependency)
# ---------------------------------------------------------------------------
...
class PartHead(nn.Module):
    """IGGT-style dense instance embedding head.
    ...
    output_dim : int
        Embedding dimension of the output per-pixel features.
    patch_size : int
    ...
    def __init__(
        ...
        # 1×1 convs: project multi-scale inputs to `features` dim.
        ...
        # Cross-attention layers for point-feature conditioning.
        ...
        # Window self-attention (operates at conv2_in_channels = features//2).
        ...
        # Window cross-attention at one fusion scale.
    ...
    def _scratch_forward(
        ...
        # Cross-attention with point features at scale 4→3.
    ...
    def forward(
        ...
        # 1. DPT fusion with optional point-feature cross-attention.
        out = self._scratch_forward(multi_scale_features, point_feat=point_feature)

        # 2. Window self-attention on fused features.
        out = out.permute(0, 2, 3, 1).contiguous()
        out = self.window_self_atten(out)
        out = out.permute(0, 3, 1, 2).contiguous()

        # 3. Interpolate to target resolution.
        ...
        # 4. Final convolutions → output_dim channels.
        out = self.output_conv2(out)

        # 5. Reshape to [B, S, C, H, W].
