...
- [04 — 跨批 query 池化 + 3D IoU 去重](issues/04-query-pooling-and-3d-dedup.md)：
  **出图了，bench 上 93 候选 → 8 个实例，每个跨全部 9 个批。** 吞掉全场的不是阈值，
  是**一个可点名的背景 slot（9 个批里都是 234，2D 就盖住 67% 画面）**；点掉它之后
  `logit>0`（模型自己的 2D 边界）直接能用，每候选最大认领量从 85.5% 掉到 8.1%。
  四条定案：① soft 集合，不 argmax（重叠只占已认领的 7.4%，最多叠 3 层）；
  ② 按**高斯集合 IoU** 合并，不按 query 余弦（余弦最好只到 F1 0.842，且阈值 0.5→0.9
  实例数 6→17 不稳；IoU 在 0.15–0.5 上稳）；③ 物性在 **model space**（对数域）按 score
  加权平均再 denormalize，`var` 与成员间 `mu_spread` **分开摆**；④ `num_ray` 小的高斯
  **不丢**（命中越少认领越少，丢了只减 recall）。三条带下游：
  ① 无人认领 87.8% 基本是背景 ⇒ **当 recall 缺口报，别塞第 0 类**，但 id5 那个
  80,511 高斯的实例是碎地板，stuff/thing 没被一刀切干净，07 号票留意；
