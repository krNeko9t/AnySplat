...
### 6. (b) 臂：config 写好，当时**不可跑**（09 落地后已解，见 §13）
...
### 13. 两臂已同时起跑（2026-09-07 17:08，09 号票落地后）

09 号票交付了 glob + `!` 取反并顺手重生成了 (b) 臂的 lock。**05 号票要的那道硬门槛逐条核过**：

| | arm (a) 全冻 | arm (b) LoRA | 差 |
|---|---|---|---|
| `freeze keyword '!*.lora.*'` | —— | `-> 192 params matched` | |
| `frozen ... params total` | **1533** | **1341** | −192 |
| `Trainable params` | **487 M** | **496 M** | +9 M |
| 优化器默认组 | 1073 | 1265 | +192 |
| lock 逐行 `-` → `T` | —— | **192 行，合计 9,437,184 参数，全部含 `.lora.`** | |
| lock 其余差异 | 仅关键词回显行 + `structure` 哈希 | | |

**两臂 step 0 的 val 读数逐字节相同**
（`log10 E 0.7872 (const 0.7825, clut 0.9575)`）⇒ 同权重、同数据、同 seed，
**对照的起点确实是同一个点**，唯一变量就是那 9.44M。

9.44M / 487M = **+1.9%**，与 01 号票逐张量数出来的对上。

跑在 `bms-39468022-001`：(a) 卡 0–3、(b) 卡 4–7，输出在
`/mnt/storage_pool/liaoyuanjun/runs/`。按 1.09 s/step 估，20k step 约 **6 小时**，
两臂同时到手。
