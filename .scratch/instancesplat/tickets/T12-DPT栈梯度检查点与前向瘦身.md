...
> **T8 关闭后回填的实测锚点**：`voxelize_ratio` 实测**恒为 1.000**（零合并，
> 2/4/8 视角 × 两子集 × 两分辨率）——R3 估算里「压缩比 [0.16, 0.6]」这项被证伪，
> GS 数 = h·w·v 线性于视角数，显存预算**不能**指望体素化压缩。零样本推理
> （b=1、backbone 冻结、bin 分辨率）峰值 3.77 / 4.50 / 6.11 GiB @N=2/4/8，
> 是「静态 + 激活」的冻结下界锚点。
