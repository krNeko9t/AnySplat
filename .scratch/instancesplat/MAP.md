...
- [AnySplat 零样本基线](tickets/T8-AnySplat零样本基线.md)：**已测**（50 val 场景，协议锁
  定在 `scripts/zeroshot_baseline_anysplat.py`）。纯重建 PSNR = **30.41 / 28.27 / 27.45 dB**
  @N=2/4/8（224 对照 31.53 / 28.70 / 27.89）；视角越多越**差**（pose-free 对齐误差累积，非 OOD）。
  探针：**`voxelize_ratio` = 0.906 / 0.840 / 0.769 @N=2/4/8**（N=8 逐场景 0.504–0.934），
  即体素化只省 8–28% 的高斯。推理峰值显存 3.77 / 4.49 / 6.08 GiB @N=2/4/8。
...
- **`voxel_size` 要不要调**：现在是 AnySplat 默认 `0.002`，论文没给这个数。体素粒度直接决定
  ...
  **T8 实测锚点**：`0.002` 下 `voxelize_ratio` = 0.906 / 0.840 / 0.769 @N=2/4/8，
  `scene_scale` 中位 0.98 —— 体素化**不是** no-op，但只压掉 8–28%。
  「靠调大体素省显存」这条路基本走不通（调大就糊实例边界），预算里不能指望它。
- **T12 做完后是否还需要降视角数**：R3 估算里不确定度最大的一项（`voxelize_ratio`，±60%）
  **已由 T8 实测打掉到 ±5%**：N=8 时 GS = 0.98M（spp）/ 0.73M（re10k），硬上界 1.20M。
  零样本推理峰值 3.77/4.49/6.08 GiB @N=2/4/8（b=1、冻结下界）——开不开体素化峰值几乎不变，
  这个锚点稳定。训练峰值仍待 T9 实测。
