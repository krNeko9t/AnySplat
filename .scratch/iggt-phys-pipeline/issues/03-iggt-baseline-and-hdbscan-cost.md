...
**2 个 stuff + 6 个真物体 + 1 个碎片。**

⚠️ **本表的玩具车与葡萄两行的高斯数原先写反了，2026-09-09 由 05 号票发现、orchestrator 核实后已改。**
决定性证据是标签文件本身：`ticket03/hdbscan/bench_labels_default.npz` 里
**id 4 = 13,434、id 6 = 7,581**。id↔物体的命名一直是对的（4 是车、6 是葡萄），
**错的是挂在它们身上的两个数**。像素占比也自洽（车 1.6% < 葡萄 2.4%）。 墙和地板各自成一整块，既没碎也没并。
除 #5 外没有簇小于全场景的 0.1%。
