...
⚠️ **本条随后被 03 号票推翻**（见地图 G9）：`sam/mask` 的 id **不跨视角一致**，
是逐帧 SAM 输出。「10」是 36 帧 id 并集的最大值，与老图的「8」（帧 00 的计数）
**是同一类错误，两个都不是场景实例数**。
本票的池化口径读数**不受影响**（两侧用同一套 id 语义，污染对称抵消，比的是协议差），
但那 10 行**不能当作实例级的物性结论**读。
