...
⚠️ **G8 后被 05 号票加严（2026-09-09）：免疫属于 `encoder_batch_size` 这个 flag，不属于视角数。**
「bench 免疫（36 视角一次前馈）」只在**整个场景确实走一次前馈**时成立。
**04 与 06 都用 `encoder_batch_size=4` trace 的 bench ⇒ 那是 9 个特征空间，不是 1 个。**
实测（同权重、其余全同）batch-4 vs batch-48：`gau_inst_feat` maxabs 3.59e-2 /
**相对 0.199**（噪声底 6e-7），**标签从 9 个实例变成 11 个，ARI 0.8708**。
（04 与 06 的池化口径读数**仍有效**——那些比较的两侧共享同一个特征空间。）
⇒ **要满足导师的「一次前馈」，必须显式把 `encoder_batch_size` 调到 ≥ 视角数**；
garden(185)/bicycle(194) 这类场景**按字面无法满足**。
...
- [03 — 官方 IGGT 权重的实例基线 + HDBSCAN 代价](issues/03-iggt-baseline-and-hdbscan-cost.md)：
  **验收对照组已就位**——`runs/ticket03/overlay_bench_default/contact_sheet.png`，
  ...
  bench **9 个实例** = 2 stuff（墙 42.6% / 地板 40.9% 像素）+ 6 个真物体 + 1 个 1,025 高斯的碎片（⚠️ 车/葡萄两行的高斯数原先写反，已按 npz 更正：id4 车 13,434、id6 葡萄 7,581）。
...
- [05 — 交付物](issues/05-deliverables.md)：**收口，导师那四条全部兑现。**
  主图 `figures/05_instance_physics_panel.jpg`：全分辨率叠加（原图在下）+ 9 行实例物性表，
  `n_gau` / `trust` / SI 的 (E,ν,ρ) / `var` 三列 / `mu_spread` 三列，
  图上直接印了「E 不精确到优于约 1.5 倍」与 `trust=LOW` 的含义。
  **并排是 null diff：与 03 基线逐字节相同（762,048 像素差 0 个，标签 ARI 1.0000）**
  ⇒「不明显更差」以最强的形式成立——挂物性头 + 多 trace 32 通道对分割的改变**恰好是零**。
  一条命令 72.2 s 跑完 bench。三个待决：**保 504×336**（448×252 会让墙/地板那一刀移 42k 高斯，
  且头在自己的训练分辨率上反而**更不自信**，「越接近训练分辨率越好」无证据支持）、
  输出改到 `/mnt/storage_pool/.../ticket05/`、**落盘 `gau_phys_feat`**（79 MB fp16，换头秒级重解码）。
  ⚠️ **② 一次前馈是 flag 的属性不是场景的属性**（bench 在 `encoder_batch_size=4` 下是 9 次前馈）。
  ⚠️ **#5（1,025 高斯）是碎片不是物体**，有数是因为它是个簇；**#8/#9 是 stuff**。
  ⚠️ **猫那一行不要当成绩卖**：头最不自信的一行，且换分辨率动了 12×。
  **garden**：残缺 PLY 上跑通不崩（88 实例，自控制组 ARI 0.9997），全程标注 partial；
  与 03 的 82 对不上（ARI 0.5638），第二个致差 flag 未定位。
  **bicycle**（完好数据，6.13 M 高斯）：跑通不崩，12 实例全带物性，
  **但地面+长椅+自行车并成一个 44.5% 的簇——不是分割的成功案例。**

## Not yet specified

- **bicycle 这类大户外场景的分割操作点**。05 号票实测：出厂的 50/10/0.06 在 bicycle 上
  把地面、长椅和自行车本身并成一个占全场景 44.5% 的簇。而 03 号票已证 50/10/0.06 是
  bench+garden 网格里唯一不退化的点 ⇒ **不存在一个跨三个场景都成立的操作点**，
  这比 03 收工时看到的图景更硬。真要做，得先想清楚是调参、调 `knn_k` 平滑，还是根本不该
  用一个全局操作点。

- **garden 与 03 号票对不上的第二个 flag**。05 号票定位到 `--trace_crop_aligned` 解释了
  覆盖率差（95.3% vs 96.25%），但 crop-aligned 的控制组给 79 个实例而非 03 的 82，
  **还有一个变量没找出来**。trace 的 flag **不进 `.pt` 的 provenance**（G10），
  这是根因；真要根治是给 `.pt` 加一份完整的 flag 快照，不是继续对数。
